...
> Critical → High, see Z-1 row); Z-2, Z-4, Z-5 remain open.** See
...
> Last updated: 2026-04-29 (SC-5 Phase B.3 — macOS agent now pins the
...
> cover the parser surface (`PublisherIdentitySpec` happy + reject
> paths including theory-driven malformed Team IDs, `PkgutilSignatureParser`
> happy + unsigned + empty + unrelated-parens-don't-match) and the six
> integration paths (matching Team ID proceeds past the gate to the
> next install step, mismatch fails closed with both expected and
> observed Team IDs in the error, unsigned pkg fails closed, signed-
> but-not-Developer-ID pkg fails closed, Authenticode-on-macOS fails
> closed, and the `RequirePackageSignature=false`-but-`publisher_identity`-
> set backward-compat angle still gates). 91 / 91 macOS .NET tests
> passing (was 77). **Phase B.2 (Windows agent `WinVerifyTrust`
> invocation) and B.4 / B.5 (cross-platform regression tests + 30/60-day
> publisher migration plan) remain open.** SC-4 still open.
>
