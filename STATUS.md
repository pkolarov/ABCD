...
> stderr warning + admin-guide subsection + regression test); Z-2,
> Z-4, and the Z-5 encryption half remain open.** See
...
> | Z-5 | ⚠ **partially closed (doc half landed 2026-04-29)** | Encrypted at rest | `dds-cli export` dumps are plaintext-CBOR (signed for integrity, not encrypted for confidentiality). **Doc-half landed 2026-04-29:** `handle_export` in [`dds-cli/src/main.rs`](dds-cli/src/main.rs) now emits an explicit stderr warning on every export — `WARNING: The dump file is signed for integrity but is NOT encrypted.` plus the "treat as Restricted material; encrypt before transit (GPG / age / FDE)" advisory — so an operator piping the dump into a courier flow sees the confidentiality posture rather than silently shipping plaintext. [`docs/DDS-Admin-Guide.md`](docs/DDS-Admin-Guide.md) §"Air-Gapped Sync" gained a new "Confidentiality posture (Z-5)" subsection enumerating the three operator-side wrappers (GPG / age, encrypted volume, wrapped channel) until the encrypted-dump variant lands. New regression test in [`dds-cli/tests/smoke.rs`](dds-cli/tests/smoke.rs) `test_export_import_round_trip` asserts the warning appears in stderr. **Still open:** the encryption half (Z-5 remediation candidate "encrypted variant — `dds-cli export --encrypt-to <pubkey>` → hybrid-PQ KEM envelope around the existing signed CBOR") rides on the Z-1 Phase B hybrid-KEM machinery landing first. |
