...
**Audit (`cargo audit`):** 0 vulnerabilities. 3 informational-only warnings
(`atomic-polyfill` RUSTSEC-2023-0089, `paste` RUSTSEC-2024-0436, `lru`
RUSTSEC-2026-0002) — unchanged from 186th pass, still unactionable without
libp2p or pqcrypto-mldsa major-version bumps.

**Gap analysis:** No documentation–code gaps found. All sample outputs
verified by prior passes; Developer Guide test count (796 / 796, 5 ignored)
and Build Health table (.NET 824 = Linux 369 + macOS 172 + Windows 283)
confirmed accurate against current binaries and test runs.

**Bug scan:** No new panicking paths found. All `unwrap()` / `expect()`
calls outside test code verified inside `#[cfg(test)]` blocks. The single
TODO(security) in `dds-node/src/service.rs:2718` is the tracked M-22
OS-bound key-wrapping item, unchanged. No other actionable TODOs in
production code.
...
`cargo test --workspace --lib` — **796 passed; 0 failed; 5 ignored** (macOS ARM64,
2026-05-24; count unchanged from 186th pass).
...
`cargo audit` — 0 vulnerabilities; 3 informational-only warnings (same
unactionable baseline as 186th pass).

---

## Fix (2026-05-24, 186th pass) — Correct stale .NET test counts in Build Health table

### Summary

Automated scheduled sweep identified stale `.NET tests` row in the Build Health table.
Passes 128–132 and commit `42282b0` added 19 new .NET tests across the three platform
agents, but the Build Health table was only updated through passes 123–127 (commit
`6b1d7a2`). The 742ba7c Rust-count-only update on 2026-05-23 carried the stale .NET
numbers forward.

### Fix

**`STATUS.md`** Build Health table and platform tables:
- `.NET tests` row: Linux 364 → **369**, macOS 166 → **172**, Windows 275 → **283**.
  Windows total: 314 → **322**. Total .NET on macOS dev host: 805 → **824**.
- `Total tests` row: 1896 → **1915** (Rust 1091 + .NET 824); all hosts 1982 → **2001**.
- macOS platform row: ".NET build + 166 tests" → ".NET build + 172 tests".
- Linux platform row: "✅ 364 C# tests" → "✅ 369 C# tests".
- Cross-platform table macOS ARM64 row: 166/364/275 → 172/369/283.

All three .NET test suites run clean on macOS dev host:
- Linux: 369 / 369 passed
- macOS: 172 / 172 passed
- Windows (non-integration): 283 / 283 passed
...
`dds-node/src/node.rs` had Phase A2 (AdmissionCert v2 + H-12 challenge-response) wired into the production paths since the 140th pass but with three gaps:

1. **No unit tests for `verify_admission_challenge`.** The helper that verifies Ed25519 and ECDSA-P256 challenge signatures in H-12 had zero test coverage, meaning a silent regression in either branch would not be caught before production.

2. **Stale comment at line 565.** The comment on the `admission_key_provider` load block said "not yet wired into H-12 (Phase A2)" — the opposite of truth after A2 shipped.

3. **Stale documentation.** `docs/hardware-bound-admission-plan.md`, `Claude_sec_review.md` §Z-2, `docs/AD-drop-in-replacement-roadmap.md` (P0-G1 gate + requirement table), and the STATUS.md zero-trust table all showed Phase A1 as the latest shipped phase and described the clone attack as still open, when Phase A2 code had already closed it for v2-cert peers.

### Fix

**`dds-node/src/node.rs`** (+8 unit tests in new `admission_challenge_tests` module, +1 comment fix):

Unit tests added:
- `ed25519_valid_signature_accepted`: generate Ed25519 key, sign a challenge, verify succeeds.
- `ed25519_wrong_signature_rejected`: sign a different message, verify fails.
- `ed25519_wrong_key_rejected`: sign with key A, verify against key B's pubkey → fails.
- `ed25519_bad_pubkey_length_rejected`: 16-byte pubkey (must be 32) → fails.
- `ed25519_bad_signature_length_rejected`: 32-byte sig (must be 64) → fails.
- `ecdsa_p256_valid_signature_accepted`: generate P-256 key, sign, DER-encode, verify succeeds.
- `ecdsa_p256_wrong_signature_rejected`: sign wrong message, verify against correct challenge → fails.
- `ecdsa_p256_bad_pubkey_rejected`: all-zero 33-byte pubkey (not on curve) → fails.

Comment fix: updated the `admission_key_provider` load block to say "Phase A2" and describe what is actually wired.

**`docs/hardware-bound-admission-plan.md`**: status block updated to "Phase A2 shipped 2026-05-21 — A3–A6 pending"; phasing table rows A1 and A2 marked ✅ with Phase A2 deliverables listed.

**`Claude_sec_review.md` §Z-2**: updated header to "Phase A2 shipped"; Phase A2 deliverable list added; attack-status section rewritten to reflect that v2-cert clone attack is now closed; remediation track updated.

**`docs/AD-drop-in-replacement-roadmap.md`**: P0-G1 gate and requirement table updated to "Phase A2 shipped".

**`STATUS.md`** zero-trust table: Z-2 row updated to "Phase A2 shipped" with A2 deliverables.

### Result

Rust workspace unit tests: **963 → 980** (+17 new tests total across this pass and the Phase A2 wire-format test batch committed alongside it; 0 failures). `cargo fmt` clean.

dds-node lib tests: **336 → 344** (+8 `admission_challenge_tests`).
dds-net lib tests: +5 Phase A2 `AdmissionRequest`/`AdmissionResponse` wire-format roundtrip tests.
dds-domain lib tests: +4 Phase A2 `issue_admission_v2` and `admission_pubkey` embed tests.

---

## Test Gap (2026-05-21, 140th pass) — dds-node: Z-2 Phase A1 KeyProvider trait + SoftwareKeyfile backend

### Gap

`dds-node/src/node.rs` loaded the admission key in `DdsNode::init()` but there were no unit tests for the `SoftwareKeyfile` backend itself.

### Fix

**`dds-node/src/key_provider.rs`** (+4 tests): `software_keyfile_stable_across_loads`, `software_keyfile_sign_verify`, `provider_kind_is_software`, `identity_handle_is_deterministic`.

### Result

Rust workspace unit tests: **959 → 963** (+4 new tests; 0 failures).
...
`platform/macos/DdsPolicyAgent.Tests/WorkerTests.cs` was missing Worker-level
tests verifying that audit-mode reconciliation correctly emits `[AUDIT]`-prefixed
directives in the reconciliation report for five enforcer categories:

1. **Stale launchd jobs** — `LaunchdEnforcer.ReconcileStaleItems` returns
   `[AUDIT] Reconcile-Unload {label}` in audit mode, but there was no
   Worker-level test confirming the job was not unloaded and the directive
   reached the `_reconciliation` report.
2. **Stale preferences** — `PreferenceEnforcer.ReconcileStaleItems` returns
   `[AUDIT] Reconcile-Delete {key}` in audit mode; same gap.
3. **Stale profiles** — `ProfileEnforcer.ReconcileStaleProfiles` returns
   `[AUDIT] Reconcile-Remove profile {id}` in audit mode; same gap.
4. **Stale software** — `SoftwareInstaller.ReconcileStalePackages` returns
   `[AUDIT] [MANUAL] Reconcile-Uninstall {id} (...)` in audit mode; same gap.
5. **Stale group memberships** — `MacAccountEnforcer.ReconcileStaleGroups` returns
   `[AUDIT] Reconcile-RemoveGroup '{user}' from '{group}'` in audit mode; same gap.

The existing `Reconciliation_StaleAccount_AuditMode_DoesNotDisable` test covered
accounts (path 2 of 6 reconciliation paths). The remaining five reconciliation paths
had correct audit-mode logic in the enforcers but no Worker-level test coverage.
This is the same pattern fixed for Linux in passes 120-122.

### Fix

**`platform/macos/DdsPolicyAgent.Tests/WorkerTests.cs`** (+5 tests):

- `Reconciliation_StaleLaunchdJob_AuditMode_DoesNotUnload`:
  pre-loads `com.dds.old-job` via `InMemoryLaunchdOperations`, seeds the managed
  launchd set, runs `PollAndApplyAsync` with `enforcement:"Audit"` and no launchd
  directives, asserts:
  - job is still loaded (Unload not called)
  - reconciliation report contains `[AUDIT]` and `com.dds.old-job`

- `Reconciliation_StalePreference_AuditMode_EmitsAuditPrefixedDeleteDirective`:
  pre-seeds a `System:com.apple.dock:autohide` preference, runs in audit mode,
  asserts:
  - preference value still exists (DeleteValue not called)
  - reconciliation report contains `[AUDIT]` and `System:com.apple.dock:autohide`

- `Reconciliation_StaleProfile_AuditMode_EmitsAuditPrefixedRemoveDirective`:
  pre-installs `com.dds.old-profile`, runs in audit mode, asserts:
  - profile still installed (Remove not called)
  - reconciliation report contains `[AUDIT]` and `com.dds.old-profile`

- `Reconciliation_StaleSoftware_AuditMode_EmitsAuditPrefixedManualEntry`:
  pre-seeds `com.example.app` in managed software, runs in audit mode, asserts:
  - reconciliation report contains both `[AUDIT]` and `com.example.app`

- `Reconciliation_StaleGroupMembership_AuditMode_DoesNotRemove`:
  pre-adds `alice` to `sudo`, pre-seeds `alice:sudo` in managed groups, runs in
  audit mode, asserts:
  - alice still in sudo (RemoveFromGroup not called)
  - reconciliation report contains `[AUDIT]` and `alice`

### Result

macOS test count: **161 → 166** (5 new tests; 0 failures).
Linux count: 359 (unchanged). Windows count: 272 (unchanged). Rust workspace: no changes.
...
> 2026-05-02 (`--encrypt-to` hybrid-KEM envelope + auto-decrypt on import); Z-2
> Phase A1 landed 2026-05-21 (KeyProvider trait + SoftwareKeyfile); Z-2
> Phase A2 landed 2026-05-21 (AdmissionCert v2 + H-12 challenge-response; A3–A6 pending);
> Z-4 remains open.** See
...
> | Z-2 | **High** ⚠ Phase A2 shipped | HW-bound identity | **Phase A1 landed 2026-05-21:** `KeyProvider` trait + `SoftwareKeyfile` backend. **Phase A2 landed 2026-05-21:** `AdmissionBody.admission_pubkey`, `AdmissionRequest.challenge` (32-byte nonce), `AdmissionResponse.challenge_signature`; `verify_admission_challenge` (Ed25519 + ECDSA-P256); `DdsNode::pending_challenges`; `allow_v1_certs` config flag; CLI `issue-self-admission` / `issue-admission` embed pubkey. 8 new unit tests. V2-cert peers now prove key possession per-handshake; clone attack closed for v2 certs. A3–A6 (TPM 2.0, SE, migration, `allow_v1_certs=false` flip) pending. See `Claude_sec_review.md` §Z-2 for full phasing. |
