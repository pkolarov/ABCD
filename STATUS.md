...
> Phase A1 landed 2026-05-21 (KeyProvider trait + SoftwareKeyfile; A2–A6 pending);
...
> | Z-2 | **High** ⚠ Phase A1 shipped | HW-bound identity | **Phase A1 landed 2026-05-21:** `dds-core::key_provider::KeyProvider` trait + `dds-node::key_provider::SoftwareKeyfile` backend in-tree; `DdsNode::admission_key_provider` field loaded in `init()` from `admission_key.bin`; WARN on software-only startup. Attack surface unchanged until Phase A2 (AdmissionCert v2 + H-12 challenge-response). See `Claude_sec_review.md` §Z-2 for the full phasing ledger. |
