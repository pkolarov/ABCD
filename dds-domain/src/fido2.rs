//! Minimal FIDO2 / WebAuthn attestation parsing and verification.
//!
//! Supports the `none` and `packed` (self-attestation, alg=-8 / Ed25519)
//! attestation statement formats. Full attestation with x5c certificate
//! chains is intentionally out of scope — DDS treats the WebAuthn
//! attestation as evidence of possession at enrollment time, then wraps
//! the credential public key inside a DDS-signed token whose chain of
//! trust flows from a Vouchsafe root, not from the FIDO metadata service.
//!
...
pub struct ParsedAttestation {
    ...
    /// Credential public key as Ed25519 verifying key (only Ed25519 supported).
...
fn parse_auth_data(auth_data: &[u8]) -> Result<AuthDataParts, Fido2Error> {
    ...
    let credential_public_key = cose_to_ed25519(&cose_value)?;
...
/// Convert a COSE_Key CBOR map to an Ed25519 verifying key.
/// Expects: kty=1 (OKP), alg=-8 (EdDSA), crv=6 (Ed25519), x=<32 bytes>.
fn cose_to_ed25519(value: &CborValue) -> Result<VerifyingKey, Fido2Error> {
    let map = value
        .as_map()
    ...
    if kty != Some(1) {
        return Err(Fido2Error::Unsupported(format!(
            "kty={kty:?}, only OKP supported"
        )));
    }
    if alg != Some(COSE_ALG_EDDSA) {
        return Err(Fido2Error::Unsupported(format!(
            "alg={alg:?}, only EdDSA supported"
        )));
    }
    if crv != Some(6) {
        return Err(Fido2Error::Unsupported(format!(
            "crv={crv:?}, only Ed25519 supported"
        )));
    }
    let x = x.ok_or_else(|| Fido2Error::Format("missing x coord".into()))?;
    if x.len() != 32 {
        return Err(Fido2Error::Format(format!("x len={}", x.len())));
    }
    let mut bytes = [0u8; 32];
    bytes.copy_from_slice(&x);
    VerifyingKey::from_bytes(&bytes).map_err(|e| Fido2Error::KeyError(e.to_string()))
}
...
fn verify_packed(
    ...
    if alg != Some(COSE_ALG_EDDSA) {
        return Err(Fido2Error::Unsupported(format!("packed alg={alg:?}")));
    }
    let sig = sig.ok_or_else(|| Fido2Error::Format("missing sig".into()))?;
    let signature = Signature::from_slice(&sig).map_err(|e| Fido2Error::KeyError(e.to_string()))?;
...
mod tests {
    ...
    fn test_none_attestation_roundtrip() {
        ...
        assert_eq!(
            parsed.credential_public_key.to_bytes(),
