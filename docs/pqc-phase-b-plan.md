...
**Status:** Partial implementation in progress — B.1–B.11 landed; B.12 open.
**Date:** 2026-04-29. **Updated:** 2026-05-01 (4th pass).
...
| B.11 | **Phase C/E observability — `dds_pq_rotation_total` landed 2026-05-01** at [`dds-node/src/telemetry.rs`](../dds-node/src/telemetry.rs). `Telemetry` grew `pq_rotation: Mutex<BTreeMap<String, u64>>` with `bump_pq_rotation / pq_rotation_snapshot / pq_rotation_count` methods; public `record_pq_rotation(reason: &str)` bumps the per-reason counter; renderer emits `dds_pq_rotation_total{reason=time|revocation|manual}`. Previous landed metrics: `dds_pq_releases_installed_total`, `dds_pq_releases_emitted_total`, `dds_pq_envelope_decrypt_total`. **Remaining B.11 work:** `dds_pq_epoch_id` (current epoch_id gauge), `dds_pq_release_request_total` (request-side counter), Phase E alert rules for non-`ok` rates — all deferred to B.12 / follow-on. | done |
