...
**Status:** Partial implementation in progress — B.1–B.8 landed; B.9–B.12 open.
**Date:** 2026-04-29. **Updated:** 2026-05-01 (3rd pass).
...
| B.11 | Phase C/E observability (metrics + alerts) — `dds_pq_epoch_id`, `dds_pq_releases_emitted_total`, `dds_pq_envelope_decrypt_total{result=ok|no_key|aead_fail}`, `dds_pq_release_request_total`, `dds_pq_rotation_total{reason=time|revocation|new_peer|manual}`. **Receive-funnel subset landed** 2026-05-01: new `dds_pq_releases_installed_total{result=ok|schema|recipient_mismatch|replay_window|kem_ct|decap|aead}` counter bumped from every exit branch of [`DdsNode::install_epoch_key_release`](../dds-node/src/node.rs). **Mint-side metric landed 2026-05-01 (this commit):** `dds_pq_releases_emitted_total{result=ok|no_kem_pk|malformed_kem_pk|not_for_self|clock_error|mint_fail|cbor_fail}` counter added to [`dds-node/src/telemetry.rs`](../dds-node/src/telemetry.rs) and bumped from every exit branch of [`DdsNode::build_epoch_key_response`](../dds-node/src/node.rs); `HELP`/`TYPE` headers always emitted (catalog-discoverable on a fresh node); `pq_releases_emitted_count(result)` test hook added; two renderer unit tests pin empty-family discoverability + value-line shape. **Decrypt-path metric fully wired (this commit):** `dds_pq_envelope_decrypt_total` now bumped from both the gossip decrypt path (B.7, landed prior) and the new sync decrypt path (`DdsNode::handle_sync_response`, B.8, this commit) — full coverage of both plaintext-over-encrypted-transport surfaces the plan tracks. The telemetry module doc-table updated to list `dds_pq_releases_emitted_total` and correct the `dds_pq_envelope_decrypt_total` source note. **Remaining B.11 work:** `dds_pq_rotation_total` (rides on B.9 rotation timer); Phase E alert rules tied to non-`ok` rate. | partial |
