...
| ID | Task | Repo Area | Deliverable |
| --- | --- | --- | --- |
| AD-08 ✅ | Gate claim path by `JoinState` instead of boolean domain join | `platform/windows/native/DdsAuthBridge/DdsAuthBridgeMain.{h,cpp}` | Landed 2026-04-26: `HandleDdsStartAuth` now gates by `GetJoinState()` *before* the WebAuthn ceremony. EntraOnly/Unknown short-circuit with `IPC_ERROR::UNSUPPORTED_HOST`; AD/Hybrid hosts with no vault entry short-circuit with `IPC_ERROR::PRE_ENROLLMENT_REQUIRED`. The previous in-worker `AUTH_FAILED` check remains as defence-in-depth but never fires under the new gate. Workgroup behaviour is unchanged: vault-backed → proceed, no vault → claim mode. |
| AD-09 ✅ | Filter DDS user list to vault-backed users on AD / hybrid | `platform/windows/native/DdsAuthBridge/DdsAuthBridgeMain.cpp`, `DdsBridgeIPC/ipc_messages.h` | Landed 2026-04-26: `IPC_RESP_DDS_USER_LIST` extended with `status_code` + `status_text` (spec §4.1) so an empty list can carry `UNSUPPORTED_HOST` instead of being indistinguishable from "no users yet". `HandleDdsListUsers` now (a) returns the status-bearing empty list on EntraOnly/Unknown, (b) intersects the dds-node list with the local vault by base64url credential_id on AD/Hybrid, and (c) **fixes a pre-existing bug**: the dds-node-failure fallback was calling `HandleListUsers`, which sends `IPC_MSG::USER_LIST`, but the DDS CP client rejects any non-`DDS_USER_LIST` message — so vault-only fallback used to silently produce an empty tile list. The new path synthesizes DDS-shape entries from vault records (subject_urn = SID, base64url credential_id, vault display_name). |
| AD-10 ✅ | Improve CP status text for unsupported and pre-enrollment-required states | `platform/windows/native/DdsCredentialProvider/CDdsCredential.cpp` | Landed 2026-04-26: CP now owns the §4.4 canonical text. New `s_rgDdsCanonicalErrorText[]` table maps the six AD-coexistence IPC codes (16..21) to (text, status icon) pairs; `GetSerializationDds` looks up the bridge-supplied `errorCode` and uses the canonical CP string instead of the bridge's free-form message. Older codes (AUTH_TIMEOUT, USER_CANCELLED, SERVICE_ERROR, …) keep the bridge text — they are not part of the AD taxonomy. STALE/CHANGE_REQUIRED/EXPIRED/PRE_ENROLLMENT show `CPSI_WARNING` (recoverable); UNSUPPORTED_HOST and ACCOUNT_NOT_FOUND show `CPSI_ERROR` (admin/reconfig). Six new standalone tests in [test_dds_bridge_selection.cpp](../platform/windows/native/Tests/test_dds_bridge_selection.cpp) pin: full coverage of codes 16..21, warning-icon mapping for the recoverable codes, error-icon mapping for unsupported/missing-account, no-mapping fallthrough for pre-AD codes, the deliberate copy share between AD_PASSWORD_CHANGE_REQUIRED and AD_PASSWORD_EXPIRED, and the deliberate copy distinction between PRE_ENROLLMENT_REQUIRED and UNSUPPORTED_HOST. |
| AD-11 ✅ | Add native tests for AD coexistence auth path | `platform/windows/native/Tests/` | Landed 2026-04-26: new [test_ad_coexistence.cpp](../platform/windows/native/Tests/test_ad_coexistence.cpp) drives the gate decision through `dds::SetJoinStateForTest` + `dds::GetCachedJoinState()` (the production cache, exposed in test builds via the new `/DDDS_TESTING` define in [build_tests.bat](../platform/windows/native/Tests/build_tests.bat)). Seven new standalone tests cover §11.2 cases 1-3 (AdJoined+vault → Proceed, AdJoined+no-vault → `PRE_ENROLLMENT_REQUIRED`, EntraOnly → `UNSUPPORTED_HOST`), the Hybrid empty-vault parity, the Unknown fail-closed path, the §9.2 Workgroup→AdJoined cache transition, and a numeric-pin guard so a future IPC renumbering breaks here. |
