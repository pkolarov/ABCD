...
- **2026-04-26** — **Phase 2 (AD-04, AD-05, AD-06, AD-07) implemented.** The
  ...
  AD-09, AD-10, and AD-11 all landed 2026-04-26 (see Phase 3 below);
...
| **AD-10** ✅ | `CDdsCredential.cpp` canonical-text table + `GetSerializationDds` failure path. | **Landed 2026-04-26.** New `s_rgDdsCanonicalErrorText[]` table inside `CDdsCredential.cpp` maps the six AD-coexistence IPC codes (16..21) to (text, status icon) pairs. `GetSerializationDds` looks up the bridge-supplied `errorCode` and uses the canonical CP string instead of the bridge's free-form message; older codes (AUTH_TIMEOUT, USER_CANCELLED, SERVICE_ERROR, …) keep the bridge text — they are not part of the §4.4 taxonomy. STALE/CHANGE_REQUIRED/EXPIRED/PRE_ENROLLMENT show `CPSI_WARNING` (recoverable); UNSUPPORTED_HOST and ACCOUNT_NOT_FOUND show `CPSI_ERROR` (admin/reconfig). The NTSTATUS → recovery text + `DDS_REPORT_LOGON_RESULT` half of AD-10 already shipped with AD-14 (see `s_rgLogonStatusInfo[]` in `ReportResult`). Six standalone tests in [test_dds_bridge_selection.cpp](../platform/windows/native/Tests/test_dds_bridge_selection.cpp) pin: full coverage of codes 16..21, warning-icon mapping, error-icon mapping, no-mapping fallthrough for pre-AD codes, the deliberate copy share between AD_PASSWORD_CHANGE_REQUIRED and AD_PASSWORD_EXPIRED, and the deliberate copy distinction between PRE_ENROLLMENT_REQUIRED and UNSUPPORTED_HOST. |
| **AD-11** ✅ | `Tests/test_ad_coexistence.cpp` (new), `build_tests.bat` (added `/DDDS_TESTING`), `test_main.cpp` (registration). | **Landed 2026-04-26.** New `test_ad_coexistence.cpp` exercises the gate decision through the *production* cache (`dds::SetJoinStateForTest` → `dds::GetCachedJoinState()`), not a re-implementation. The unified test build (`build_tests.bat` → `dds_native_tests.exe` → `run_all_tests.bat`) gains `/DDDS_TESTING`, exposing the seam — production binaries (`DdsAuthBridge.vcxproj` etc.) still compile without it, so `SetJoinStateForTest` is absent from shipped artifacts. Seven tests cover the three §11.2 cases plus Hybrid empty-vault parity, Unknown fail-closed, the §9.2 Workgroup→AdJoined re-probe transition, and a numeric pin on `IPC_ERROR::PRE_ENROLLMENT_REQUIRED=19` / `UNSUPPORTED_HOST=20`. The pre-existing `ad08_*` / `ad09_*` / `ad10_*` standalone tests in `test_dds_bridge_selection.cpp` still pin the pure-decision and CP-text invariants — the AD-11 file complements them by validating the cache machinery itself. A separate `build_test_ad_coexistence.bat` was *not* added because the unified `build_tests.bat`/`run_all_tests.bat` already drive every native test in this binary; STATUS notes the convention explicitly. |
