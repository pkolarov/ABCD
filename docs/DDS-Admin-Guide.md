...
| `self_update_apply` | bool | `true` | **Phase D.6** — whether the node should apply inbound `DdsSelfUpdateDocument` update manifests. When `false`, the node still ingests and propagates the manifest (so opted-in peers receive it) but logs "self-update available, apply disabled by config" and skips the install step. Set to `false` for air-gapped or regulated deployments that maintain their own change-control pipeline. When `true` (default), the node evaluates the rollout policy (Phase D.3): `Staged` rollouts use `SHA-256(PeerId \|\| JTI) mod 100` for deterministic canary cohort assignment; `Pinned` rollouts apply only when the installed version matches the allow-list; `Halt` blocks all pending updates. When the rollout decision is `ApplyNow`, the node fetches the artifact, verifies its SHA-256 and OS-vendor signature (Phase D.4), stages it to a SYSTEM-only DACL cache, and invokes the platform-native installer (`msiexec /quiet` on Windows, `installer -pkg` on macOS). |
...
| `dds_sync_payloads_rejected_total` | counter | `reason=legacy_v1\|publisher_capability\|publisher_identity\|replay_window\|signature\|duplicate_jti\|graph` | Pre/post-apply sync rejections: `legacy_v1` = pre-v2 wire format; `publisher_capability` = issuer cert lacks the software-publisher capability; `publisher_identity` = `SoftwareAssignment.publisher_identity` field is malformed or does not match any allowlisted publisher (SC-5 Phase B.1 gate); `replay_window` = JTI outside the anti-replay window; `signature` = Ed25519 signature verification failure; `duplicate_jti` = JTI already seen; `graph` = graph-apply conflict. |
...
| `dds_pq_epoch_id` | gauge | — | Current epoch_id of the local node's AEAD epoch key. An advancing epoch_id confirms rotation is occurring; a static value on a peer indicates it has not received a new release (correlate with `dds_pq_releases_installed_total`). |
| `dds_pq_releases_installed_total` | counter | `result=ok\|schema\|recipient_mismatch\|replay_window\|bad_peer_id\|no_inline_pubkey\|bad_pubkey\|bad_sig\|kem_ct\|decap\|aead` | Inbound EpochKeyRelease install outcomes. `ok` = decap + AEAD unwrap succeeded; `decap` / `aead` = KEM or AEAD failure (key material mismatch or tampering); `bad_sig` = publisher Ed25519 signature rejected; `replay_window` = release older than 7 days. |
| `dds_pq_releases_emitted_total` | counter | `result=ok\|no_kem_pk\|malformed_kem_pk\|not_for_self\|clock_error\|mint_fail\|cbor_fail` | Outbound EpochKeyRelease mint-and-emit outcomes per recipient. `no_kem_pk` = peer cert lacks a KEM pubkey (pre-v3 cert); `ok` = release minted and sent. Elevated `no_kem_pk` means peers have not yet upgraded to v3 admission certs. |
| `dds_pq_release_requests_total` | counter | `result=sent\|cooldown\|not_admitted\|malformed_peer_id` | EpochKeyRequest outbound outcomes from the `no_key` late-join recovery path. `cooldown` = request suppressed by 30 s per-publisher cooldown; sustained `sent` rate > 0.1/s triggers `DdsPqcKeyRequestSpike`. |
