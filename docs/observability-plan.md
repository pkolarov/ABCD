...
|---|---|---|
| `dds-cli stats` | ✅ | Composes `/v1/status` + `/v1/audit/entries` into one snapshot — peer ID + uptime, connected peers, trust-graph + store sizes, **per-redb-table store bytes** (the same `dds_store_bytes{table=...}` snapshot the Prometheus gauge reads, plumbed through `/v1/status` so the CLI does not have to scrape `/metrics`), audit chain length + head age + head action. Pretty-prints by default; `--format json` emits a single JSON object for scripting / Prometheus textfile fallbacks. The text output draws `(unsupported)` for older nodes that omit the field and `(none)` for backends that report an empty map (`MemoryBackend`); the JSON output omits the `store.bytes` key entirely on older nodes so existing scripts keep parsing. `last admission failure` remains deferred — `dds_admission_handshakes_total{result="fail"}` is a since-boot counter rather than a timestamp, so a meaningful surfacing would need a separate "last failure" gauge or a `/metrics` scrape. |
