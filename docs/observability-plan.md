...
(`dds_gossip_messages_dropped_total{reason=unadmitted|unknown_topic|decode_error|topic_kind_mismatch|enc_v3_no_key|enc_v3_aead_fail|enc_v3_plaintext_rejected}` —
bumped from the seven pre-decode drop sites in
...
| `dds_gossip_messages_dropped_total` | counter | `reason=unadmitted|unknown_topic|decode_error|topic_kind_mismatch|enc_v3_no_key|enc_v3_aead_fail|enc_v3_plaintext_rejected` | Pre-decode drops — bumped from the H-12 unadmitted-relayer drop in [`DdsNode::handle_swarm_event`](../dds-node/src/node.rs) and the six early-exit branches of [`DdsNode::handle_gossip_message`](../dds-node/src/node.rs) (`unknown_topic` = topic hash not in the local subscription set; `decode_error` = `GossipMessage::from_cbor` rejected the payload; `topic_kind_mismatch` = decoded variant did not match the topic family; `enc_v3_no_key` / `enc_v3_aead_fail` = PQC envelope key missing or AEAD failure — paired with `dds_pq_envelope_decrypt_total`; `enc_v3_plaintext_rejected` = plaintext envelope on an enc-v3 domain). The catalog originally named the labels `unadmitted|invalid_token|duplicate|backpressure`; the latter three describe *post-decode* drop conditions inside the `ingest_*` paths and are already covered by `dds_audit_entries_total{action=*.rejected}` (signature / validation / duplicate-JTI rejections all funnel through the audit chain), so v1 partitions the pre-decode surface only. A future follow-up that wires a gossipsub backpressure hook can add `reason=backpressure` without renaming the metric. | ✅ |
| `dds_sync_pulls_total` | counter | `result=ok|fail` | Outbound anti-entropy pull outcomes — bumped from the resolution branches of [`DdsNode::handle_sync_event`](../dds-node/src/node.rs). `ok` = admitted peer's `Message::Response` was processed by `handle_sync_response` (zero payloads still counts as `ok` — the pull resolved, the network simply converged). `fail` = `OutboundFailure` event (timeout / connection closed / dial failure / codec error) or the H-12 unadmitted-peer response drop (response received but the peer is no longer in `admitted_peers`, so its payloads are discarded without applying any state). Per-peer cooldown skips inside `try_sync_with` are *not* counted — no request goes on the wire so there is no outcome to partition. Inbound responder-side outcomes are counted separately under `dds_sync_serves_total`. | ✅ |
| `dds_sync_serves_total` | counter | `result=ok\|unadmitted\|channel_closed` | Inbound anti-entropy serve outcomes (responder side) — bumped from the `RrMessage::Request` arm of [`DdsNode::handle_sync_event`](../dds-node/src/node.rs). `ok` = admitted peer's request was answered successfully via `send_response`; `unadmitted` = H-12 gate: peer not in `admitted_peers`, channel dropped without a response; `channel_closed` = admitted peer but `send_response` returned `Err` — channel closed before the response was queued. `InboundFailure` and `ResponseSent` events are not counted (logged at debug level). Complements `dds_sync_pulls_total` with the responder-side view, enabling an operator to distinguish inbound load from outbound pull activity. | ✅ |
...
| **PQC / key management** | | | | |
| `dds_pq_envelope_decrypt_total` | counter | `result=ok\|no_key\|aead_fail` | Phase B enc-v3 gossip/sync envelope AEAD decrypt outcomes — bumped by [`record_pq_envelope_decrypt`](../dds-node/src/telemetry.rs) at every exit branch of the envelope decrypt path in [`DdsNode::handle_gossip_message`](../dds-node/src/node.rs) and [`DdsNode::handle_sync_response`](../dds-node/src/node.rs). `no_key` = epoch key for the message's `epoch_id` not yet received (self-heals via `EpochKeyRequest` within ~30 s; the same event surfaces as `enc_v3_no_key` in `dds_gossip_messages_dropped_total`); `aead_fail` = ciphertext tampered or wrong epoch key. Sustained `no_key` or `aead_fail` activity triggers `DdsPqcDecryptFailureSpike`. | ✅ |
| `dds_pq_rotation_total` | counter | `reason=time\|revocation\|manual` | Epoch-key rotation triggers — bumped by [`record_pq_rotation`](../dds-node/src/telemetry.rs) at every rotation trigger in [`DdsNode::rotate_and_fan_out`](../dds-node/src/node.rs). `time` = scheduled cadence (default 24 h, configurable via `epoch_rotation_secs`); `revocation` = admission revocation triggered early rotation; `manual` = `POST /v1/pq/rotate` operator override. Non-zero `revocation` rate indicates the domain is actively revoking enc-v3 peers. | ✅ |
| `dds_pq_epoch_id` | gauge | — | Current `epoch_id` of the local node's AEAD epoch key — set by [`record_pq_epoch_id`](../dds-node/src/telemetry.rs) at init and on every [`DdsNode::rotate_and_fan_out`](../dds-node/src/node.rs) call. An advancing value confirms rotation is occurring; a static value on a peer indicates it has not received a new `EpochKeyRelease` (correlate with `dds_pq_releases_installed_total`). | ✅ |
| `dds_pq_releases_installed_total` | counter | `result=ok\|schema\|recipient_mismatch\|replay_window\|bad_peer_id\|no_inline_pubkey\|bad_pubkey\|bad_sig\|kem_ct\|decap\|aead` | Inbound `EpochKeyRelease` install outcomes — bumped by [`record_pq_release_installed`](../dds-node/src/telemetry.rs) at every exit branch of [`DdsNode::install_epoch_key_release`](../dds-node/src/node.rs). `ok` = ML-KEM decap + AEAD unwrap succeeded and the epoch key is installed; `decap` / `aead` = KEM or AEAD failure (key material mismatch or tampering); `bad_sig` = publisher Ed25519 signature rejected; `replay_window` = release older than 7 days; `schema` = CBOR decode error. Elevated non-`ok` rates indicate key-distribution problems — correlate with `dds_pq_rotation_total` on the publisher side. | ✅ |
| `dds_pq_releases_emitted_total` | counter | `result=ok\|no_kem_pk\|malformed_kem_pk\|not_for_self\|clock_error\|mint_fail\|cbor_fail` | Outbound `EpochKeyRelease` mint-and-emit outcomes per recipient — bumped by [`record_pq_releases_emitted`](../dds-node/src/telemetry.rs) at every exit branch of [`DdsNode::build_epoch_key_response`](../dds-node/src/node.rs). `no_kem_pk` = peer cert lacks a KEM public key (pre-v3 admission cert — peer has not yet upgraded); `ok` = ML-KEM encap + AEAD seal succeeded and the release was sent. Elevated `no_kem_pk` means peers have not yet rotated to v3 admission certs. | ✅ |
| `dds_pq_release_requests_total` | counter | `result=sent\|cooldown\|not_admitted\|malformed_peer_id` | Outbound `EpochKeyRequest` outcomes from the `no_key` late-join recovery path — bumped by [`record_pq_release_request`](../dds-node/src/telemetry.rs) at every exit branch of [`DdsNode::try_epoch_key_request`](../dds-node/src/node.rs). `cooldown` = request suppressed by 30 s per-publisher cooldown (prevents request storms on reconnect); `not_admitted` = target peer is not in `admitted_peers`; `sent` = request dispatched via gossip. Sustained `sent` rate > 0.1/s triggers `DdsPqcKeyRequestSpike`. | ✅ |
| **Trust graph** | | | | |
