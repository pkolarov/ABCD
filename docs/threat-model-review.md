...
| 21 | **No fleet self-update + no provenance (Z-8)** ⚠ partially closed (Phase C.3 + C.4 landed) — no `dds-update` / `DdsSelfUpdateDocument` / Policy-Agent self-update path (verified by grep across all language sources); no SLSA provenance, no SBOM. **`cargo audit` (C.4, 2026-04-29):** `audit` job in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs on every PR and push to `main`, exiting non-zero on any RUSTSEC vulnerability advisory (three upstream-blocked informational warnings tracked in [`security-gaps.md`](../security-gaps.md) — down from eight as of 2026-05-25). **`cargo-vet` (C.3, 2026-05-02):** `cargo vet` baseline wired into CI; Mozilla audit import; `vet` job fails on any new unreviewed dependency. Security patches still do not propagate without manual MSI/pkg redeployment; CI compromise still has no provenance trail to detect after the fact. **Remaining:** Phase C.1 (SLSA), C.2 (SBOM), C.5 (Sigstore) + Phase D (multi-sig fleet self-update, staged canary, dedicated capability). Implementation plan: [supply-chain-plan.md](supply-chain-plan.md). | **Medium** | (new supply-chain section) |
