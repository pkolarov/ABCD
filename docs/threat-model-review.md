...
| 20 | **Asymmetric managed-software signature verification (Z-7)** ❌ open 2026-04-26 — Windows `WindowsSoftwareOperations.cs` has no `WinVerifyTrust` (hash-only); macOS `pkgutil --check-signature` is opt-in via `RequirePackageSignature` (default off). C-3's DDS-trust-root chain is solid; the OS-vendor-trust complement is missing. Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phase B. | **High** | (new supply-chain section) |
