...
| 21 | **No fleet self-update + no provenance (Z-8)** ⚠ partially closed 2026-04-29 (Phase C.4 — `cargo audit` in CI landed) — no `dds-update` / `DdsSelfUpdateDocument` / Policy-Agent self-update path (verified by grep across all language sources); no SLSA provenance, no SBOM, no `cargo-vet`. **`cargo audit` half landed 2026-04-29:** new `audit` job in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) installs `cargo-audit` via `taiki-e/install-action@v2` and runs `cargo audit` on every PR and every push to `main`, exiting non-zero on any RUSTSEC vulnerability advisory (eight upstream-blocked informational warnings tracked in [`security-gaps.md`](../security-gaps.md)). Security patches still do not propagate without manual MSI/pkg redeployment; CI compromise still has no provenance trail to detect after the fact. Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phase C.1 (SLSA), C.2 (SBOM), C.3 (`cargo-vet`), C.5 (Sigstore) + Phase D (multi-sig fleet self-update, staged canary, dedicated capability). | **Medium** | (new supply-chain section) |
