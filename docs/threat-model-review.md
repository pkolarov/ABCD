...
| Risk | Severity | Mitigation |
|------|----------|------------|
| **Transport handshake is not post-quantum (Z-1)** ⚠ partially closed 2026-04-28 — Noise XX still uses X25519 DH; libp2p QUIC still uses rustls/ECDHE. Harvest-Now-Decrypt-Later on transport remains open. **`AdmissionCert` and `AdmissionRevocation` are now hybrid Ed25519 + ML-DSA-65 (FIPS 204) under Z-1 Phase A** (`Domain.pq_pubkey` / `*.pq_signature` with `dds-admission-v2/mldsa65\0` and `dds-revocation-v2/mldsa65\0` domain separators in `dds-domain::domain`); a v2-hybrid domain rejects any cert or revocation lacking the PQ component. Transport handshake portion remains. | **High** | Phase A landed 2026-04-28 — covers remediation candidate (3). Remaining: (1) hybrid Noise upstream (rust-libp2p tracking issue rs/9595, no feature flag yet); (2) interim per-message hybrid-PQ KEM envelope on gossip / sync payloads. Until at least one of (1)/(2) lands, README/whitepaper "quantum-resistant by default" must still be qualified for the transport channel. See [Claude_sec_review.md](../Claude_sec_review.md) Z-1. |
...
| 14 | **Transport handshake post-quantum gap (Z-1)** ⚠ partially closed 2026-04-28 — `AdmissionCert` + `AdmissionRevocation` are now hybrid Ed25519+ML-DSA-65 (Z-1 Phase A); transport handshake still classical (Noise X25519 / QUIC ECDHE), Harvest-Now-Decrypt-Later remains for gossip/sync payloads. | **High** | §4 |
...
| 16 | **Audit emission unwired (Z-3)** ❌ open 2026-04-26 — chain mechanism + signature verify are correct, but `DdsNode::emit_local_audit` has zero production call-sites. Implementation plan: [observability-plan.md](observability-plan.md) Phase A. | **High** | (new audit section) |
...
| 18 | **`dds-cli export` dumps plaintext (Z-5)** ⚠ partially closed (doc half landed 2026-04-29) — signed for integrity (M-16) but not encrypted; air-gap sync ships plaintext directory state. **Doc-half landed 2026-04-29:** `handle_export` now emits an explicit stderr advisory (`WARNING: The dump file is signed for integrity but is NOT encrypted.` + treat-as-Restricted guidance + GPG / age / FDE wrapper hint) on every export, and [DDS-Admin-Guide.md](DDS-Admin-Guide.md) §"Air-Gapped Sync" gained a new "Confidentiality posture (Z-5)" subsection enumerating the operator-side wrappers; new regression test in `dds-cli/tests/smoke.rs` pins the warning. **Encryption half (`dds-cli export --encrypt-to <pubkey>` → hybrid-PQ KEM envelope) remains open** and rides on Z-1 Phase B (per-message hybrid-KEM envelope on gossip + sync) landing first so the export path can reuse the same KEM module. | **Medium** | (new export section) |
