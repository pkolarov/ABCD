...
| 20 | **Asymmetric managed-software signature verification (Z-7)** ⚠ partially closed 2026-04-28 (Phase B.1 schema landed) — Windows `WindowsSoftwareOperations.cs` still has no `WinVerifyTrust` (hash-only); macOS `pkgutil --check-signature` still does not pin a Team ID. **`SoftwareAssignment` now carries an optional `publisher_identity: Option<PublisherIdentity>` field** with `Authenticode { subject, root_thumbprint }` and `AppleDeveloperId { team_id }` variants (`#[serde(default, skip_serializing_if = "Option::is_none")]` for v1 wire-byte parity). `PublisherIdentity::validate()` enforces the field-level invariants so empty / wrong-shape values fail closed at the schema layer. C-3's DDS-trust-root chain is solid; the OS-vendor-trust complement is now schema-ready and the agent-side wiring (Phase B.2 / B.3) remains the open piece. Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phase B. | **High** | (new supply-chain section) |
| 21 | **No fleet self-update + no provenance (Z-8)** ❌ open 2026-04-26 — no `dds-update` / `DdsSelfUpdateDocument` / Policy-Agent self-update path (verified by grep across all language sources); no SLSA / SBOM / `cargo-vet` / `cargo audit` in CI. Security patches do not propagate; CI compromise has no detection trail. Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phases C + D (multi-sig admission, staged canary, dedicated capability). | **Medium** | (new supply-chain section) |
