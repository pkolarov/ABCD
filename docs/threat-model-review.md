...
| Risk | Severity | Mitigation |
|------|----------|------------|
| **Transport handshake is not post-quantum (Z-1)** ⚠ partially closed (Phase A 2026-04-28 + Phase B 2026-05-01) — **Phase A:** `AdmissionCert` + `AdmissionRevocation` are hybrid Ed25519+ML-DSA-65 (FIPS 204); v2-hybrid domains reject certs/revocations lacking the PQ component. **Phase B (complete 2026-05-01):** gossip and sync payloads are now AEAD-encrypted under a per-publisher ML-KEM-768 epoch key (`enc-v3` envelope); Harvest-Now-Decrypt-Later on *content* is closed at the application layer. **Remaining:** Noise XX transport handshake and QUIC still use classical DH (X25519 / ECDHE); session keys for recorded transport traffic are not PQ-safe. Full transport PQ awaits hybrid-Noise upstream (rust-libp2p tracking issue rs/9595). See [Claude_sec_review.md](../Claude_sec_review.md) Z-1. | **High** | Phase A covered remediation candidate (3); Phase B closed candidate (2). Remaining: (1) hybrid Noise upstream. README/whitepaper "quantum-resistant by default" now qualified for the transport-handshake layer only. |
| **No message-level encryption** — gossip messages (tokens, sync payloads) are CBOR-encoded but not encrypted at the application layer. | ~~Low~~ **Closed by Z-1 Phase B (2026-05-01)** | **CLOSED.** Gossip and sync payloads are now AEAD-encrypted under per-publisher ML-KEM-768 epoch keys (`enc-v3` envelope). The classical-adversary confidentiality risk is closed; the remaining PQ gap is the transport handshake layer tracked under Z-1. |
...
| 14 | **Transport handshake post-quantum gap (Z-1)** ⚠ partially closed (Phase A 2026-04-28 + Phase B 2026-05-01) — Phase A: `AdmissionCert` + `AdmissionRevocation` hybrid Ed25519+ML-DSA-65. Phase B: gossip/sync AEAD-encrypted under per-publisher ML-KEM-768 epoch key; application-layer Harvest-Now-Decrypt-Later closed. Remaining: Noise XX / QUIC transport handshake still classical (rs/9595). | **High** | §4 |
| 15 | **Hardware-bound identities (Z-2)** ⚠ partially closed (Phase A1+A2+A4+A6-partial 2026-05-22) — `KeyProvider` trait + `SoftwareKeyfile` backend (A1); AdmissionCert v2 wire format + H-12 challenge-response step to defeat node-clone attacks (A2); Apple Secure Enclave backend `AppleSecureEnclaveKeyProvider` (A4); `provision-admission-key` / `rotate-admission-key` CLI subcommands (A6-partial). `network.admission_key_backend = "secure-enclave"` config field live. **Remaining:** TPM 2.0 backend (A3, Linux + Windows), Ed25519 path on capable TPMs (A5), `allow_v1_certs = false` default flip (A6-full — blocked on A3+A4 fleet rollout). libp2p PeerId still software-resident on non-SE hosts. See [hardware-bound-admission-plan.md](hardware-bound-admission-plan.md). | **High** | §1 / §2 |
| 16 | ~~**Audit emission unwired (Z-3)**~~ ✅ closed 2026-04-26 follow-up #17 — `LocalService::emit_local_audit` and `DdsNode::emit_audit_from_ingest` now stamp the local chain on every state-mutating HTTP action and every gossip-ingest rejection branch; signed `reason` field added for SIEM consumers. See [observability-plan.md](observability-plan.md) Phase A and [Claude_sec_review.md](../Claude_sec_review.md) Z-3. | ~~**High**~~ | (new audit section) |
...
| 18 | ~~**`dds-cli export` dumps plaintext (Z-5)**~~ ✅ closed 2026-05-02 — Doc-half landed 2026-04-29 (stderr advisory + Admin-Guide subsection + regression test). **Encryption half landed 2026-05-02:** `dds export --encrypt-to <hex-pubkey>` wraps the signed CBOR in a hybrid X25519+ML-KEM-768 KEM envelope (ChaCha20-Poly1305 AEAD, `b"dds-export-v1"` AAD); `dds import` auto-detects and decrypts; `dds pq status` prints the hex KEM pubkey. Regression test `test_export_import_encrypted_round_trip` in `dds-cli/tests/smoke.rs` covers the full round-trip. See [Claude_sec_review.md](../Claude_sec_review.md) Z-5. | ~~**Medium**~~ | (new export section) |
| 19 | **DDS releases unsigned in practice (Z-6)** ❌ open 2026-04-26 — MSI Authenticode is conditional on a `SIGN_CERT` secret that has never been provisioned; macOS pkg is not Developer-ID-signed and not notarized; CLI/FFI ship unsigned. Promotes §3's "pending Authenticode" recommendation to Critical. Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phase A. | **Critical** | §3 |
| 20 | **Asymmetric managed-software signature verification (Z-7)** ⚠ partially closed 2026-04-29 (Phase B.1 schema + ingest gate + Phase B.2 Windows agent + Phase B.3 macOS agent + Phase B.4 cross-platform tests landed) — `SoftwareAssignment` carries an optional `publisher_identity: Option<PublisherIdentity>` field with `Authenticode { subject, root_thumbprint }` and `AppleDeveloperId { team_id }` variants (`#[serde(default, skip_serializing_if = "Option::is_none")]` for v1 wire-byte parity); `PublisherIdentity::validate()` is wired into both `LocalService::list_applicable_software` (read-side) and an ingest-time gate at gossip (`DdsNode::ingest_operation`) and sync (`DdsNode::handle_sync_response`, `dds_sync_payloads_rejected_total{reason="publisher_identity"}`) so malformed entries fail closed before reaching peers. **Phase B.2 (Windows agent):** `SoftwareInstaller.ApplyInstallAsync` now routes the staged installer through `IAuthenticodeVerifier` (production `WinTrustAuthenticodeVerifier` calls `WinVerifyTrust(WINTRUST_ACTION_GENERIC_VERIFY_V2)` plus `X509Certificate2` for signer-subject and chain-root SHA-1 thumbprint pinning); the gate runs whenever **either** `RequirePackageSignature` is true **or** `publisher_identity` is set, closing the silent-downgrade window. **Phase B.3 (macOS agent):** `SoftwareInstaller` parses `pkgutil --check-signature` via `PkgutilSignatureParser` and pins the leaf-cert Team ID against `AppleDeveloperId { team_id }`. **Phase B.4:** bilateral test matrix mirrored on both agents (`phase_b2_*` / `phase_b3_*`) covering matching subject / mismatch / unsigned / wrong-platform pin / missing thumbprint / `RequirePackageSignature=false`-but-pinned-still-gates / no-`publisher_identity`-directive backward-compat. **Phase B.5 (publisher migration cutover, 30-day warn → 60-day hard-fail) remains open** and is gated on Phase A (provisioning DDS's own Windows code-signing cert + Apple Developer ID + notarization) shipping first. Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phase A → Phase B.5. | **High** | (new supply-chain section) |
| 21 | **No fleet self-update + no provenance (Z-8)** ⚠ partially closed (Phase C.1–C.5 all landed 2026-05-02) — no `dds-update` / `DdsSelfUpdateDocument` / Policy-Agent self-update path (verified by grep across all language sources). **`cargo audit` (C.4, 2026-04-29):** `audit` job in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs on every PR and push to `main`, exiting non-zero on any RUSTSEC vulnerability advisory (three upstream-blocked informational warnings tracked in [`security-gaps.md`](../security-gaps.md) — down from eight as of 2026-05-25). **`cargo-vet` (C.3, 2026-05-02):** `cargo vet` baseline wired into CI; Mozilla audit import; `vet` job fails on any new unreviewed dependency. **SLSA Level 3 provenance (C.1, 2026-05-02):** `slsa-framework/slsa-github-generator` provenance attestation job wired into both `msi.yml` and `pkg.yml`; every release MSI and pkg ships a signed `.intoto.jsonl` attestation. **SBOM (C.2, 2026-05-02):** `sbom` job in `ci.yml` runs `cargo cyclonedx --format json --all` and uploads a CycloneDX JSON artifact on every CI run. **Sigstore signing (C.5, 2026-05-02):** `sign` jobs in `msi.yml` and `pkg.yml` use keyless `cosign sign-blob` (Fulcio + Rekor) on tag builds, producing a `.bundle` file (cert + signature + tlog entry) alongside each artifact. Security patches still do not propagate without manual MSI/pkg redeployment; Phase D fleet self-update remains unimplemented. **Remaining:** Phase D only (multi-sig fleet self-update, staged canary, dedicated `dds:dds-self-update-publisher` capability). Implementation plan: [supply-chain-plan.md](supply-chain-plan.md) Phase D. | **Medium** | (new supply-chain section) |
