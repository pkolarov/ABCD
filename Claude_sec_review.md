...
| L-3 | ✅ Fixed (pending verify) | `NamedTempFile::persist` for key and admin-key writes. **2026-04-29 follow-on:** the same idiom now also covers the four `dds-node::domain_store` save paths (`save_domain_file`, `save_domain_key`, `save_admission_cert`, `save_domain_key_fido2`) which previously called `std::fs::write(path, …)` directly. New private helper `atomic_write_owner_only(path, &buf)` factors the `tempfile::NamedTempFile::new_in(parent)` + write + `set_owner_only_permissions(tmp)` + `tmp.persist(path)` sequence so a crash mid-write can no longer leave a torn `domain.toml`, `domain_key.bin`, or `admission.cbor` on disk. Pin: new regression test `dds-node::domain_store::tests::save_paths_are_atomic_and_owner_only` asserts (a) target file mode `0o600`, (b) no stray temp leftovers in the parent directory after the rename, and (c) the helper is overwrite-safe across two consecutive saves. **2026-04-29 follow-on (admission revocation file):** `dds-node::admission_revocation_store::save_revocation_file` (used by `dds-node revoke-admission --out` to write the per-revocation CBOR file operators distribute out-of-band) was likewise calling `std::fs::write(path, …)` despite a docstring claiming "atomic on Unix"; replaced with the same `tempfile::NamedTempFile::new_in(parent)` + `tmp.persist(path)` sequence (no chmod — revocations are public-by-design, only the path operator chooses for the dump-file matters; the in-memory store's own `save` path was already atomic with `0o600` since it persists the local revocation list). Pin: new regression test `dds-node::admission_revocation_store::tests::save_revocation_file_is_atomic_and_overwrite_safe` asserts (a) no stray `.tmp*` siblings after the persist, (b) round-trips the revocation through `load_revocation_file` byte-for-byte, and (c) overwrite-safe across two consecutive saves on the same path. Closes the L-3 "+ siblings" qualifier — every CBOR/key writer in the tree now uses atomic-rename. **2026-04-29 follow-on (device binding store):** `dds-node::device_binding::DeviceBindingStore::persist` (the M-8 TOFU `(device_urn, caller-principal)` map persisted to `<data_dir>/device_bindings.json`) was the last writer in the tree still applying owner-only permissions *after* `tempfile::NamedTempFile::persist`, leaving a brief window where the freshly-renamed file was observable at the umask-default `0o644` on Unix, and skipping the Windows protected DACL entirely. Replaced with the canonical `tempfile::NamedTempFile::new_in(parent)` + `crate::file_acl::restrict_to_owner(tmp.path())` *before* `tmp.persist(path)` + `restrict_to_owner(path)` after — so the JSON map of admitted device URNs to their bound caller principals (UID on Unix / SID on Windows) is owner-only at the instant it appears under its target name. Pin: new regression test `dds-node::device_binding::tests::persist_is_atomic_and_owner_only` asserts (a) no stray `.tmp*` siblings after the persist, (b) target file mode `0o600` on Unix, (c) the helper is overwrite-safe across two consecutive `tofu_bind` calls on the same path, and (d) both bindings reload byte-for-byte after the second persist. Windows DACL coverage rides on `file_acl::restrict_to_owner` which already mirrors the SDDL idiom from `identity_store::set_owner_only_permissions` and `domain_store::set_owner_only_permissions`. |
...
| L-16 | ✅ Fixed (pending verify) | `AppliedStateStore.WriteToDisk` applies the restricted DACL to the `.tmp` file BEFORE `File.Move`. Same-volume `File.Move` is a metadata rename that preserves the source DACL, so the final `applied-state.json` never observably exists with the inherited parent ACL — closes the write-time race. Fail-closed on Windows: if `SetWindowsDacl` throws, the tmp file is deleted and the exception propagates, so the caller (holding `_lock`) sees the write fail. On Windows the DACL grants `FullControl` only to `LocalSystem` and `BUILTIN\Administrators` with inheritance disabled; `OperatingSystem.IsWindows()` short-circuits the helper on non-Windows (the agent never ships there in production). Added `System.IO.FileSystem.AccessControl` to the csproj, guarded by `TargetFramework`. **2026-05-01 follow-on (CI fix):** the helper used to roll DACL + owner-change into a single `info.SetAccessControl(security)`, which threw `InvalidOperationException` (SE_INVALID_OWNER) under any caller that did not hold `SeRestorePrivilege` — including the GitHub-Actions xUnit runner under `BUILTIN\Administrators` (privilege is disabled by default on non-SYSTEM tokens). All 12 non-trivial `AppliedStateStoreTests` cases failed identically and CI had been red on every push since `48ee29c`. The DACL apply was split from the owner-change in both `AppliedStateStore.SetWindowsDacl` and the mirror helper `WindowsSoftwareOperations.ApplyWindowsDacl`: the DACL (the load-bearing security boundary) is applied unconditionally; the owner-change to `LocalSystem` is now best-effort and wrapped in `try/catch` for `InvalidOperationException` / `UnauthorizedAccessException` / `PrivilegeNotHeldException` so a non-`SeRestorePrivilege` caller still gets the locked-down DACL. Production unchanged — the agent runs as `LocalSystem` and always holds the privilege. New regression test `Save_applies_protected_dacl_even_when_owner_change_is_denied` (Windows-only via `[SupportedOSPlatform("windows")]`) asserts `AreAccessRulesProtected = true` plus the two `FullControl` ACEs (`LocalSystemSid` + `BuiltinAdministratorsSid`) on the persisted file. |
...
[dds-node/src/identity_store.rs:81](dds-node/src/identity_store.rs#L81) — `create_dir_all` uses default `0o755`. Set `0o700` after create. **Fixed (2026-04-29) for the implicit "+ siblings" branch:** `dds-node/src/domain_store.rs` now mirrors the same `#[cfg(unix)]` `set_permissions(parent, 0o700)` idiom in `save_domain_key` (the encrypted-or-plain v=1/v=2/v=4/v=5 secret), `save_domain_key_fido2` (v=3 hmac-secret-protected secret), `save_domain_file`, and `save_admission_cert` via a new private `tighten_parent_dir_perms` helper. Previously only `identity_store::save` and `p2p_identity::save` carried the fix, so a fresh data-dir touched first by `init-domain` (which runs through `domain_store::save_*` before the node identity is generated) was left at the umask-default `0o755`. New regression test `save_domain_key_tightens_parent_dir_to_owner_only` pre-loosens the temp dir to `0o755` and asserts the save call resets it to `0o700`. Windows is a no-op — the data-dir DACL applied by the MSI custom action `CA_RestrictDataDirAcl` plus the per-file DACL via `file_acl::restrict_to_owner` are the authoritative perimeters there.
