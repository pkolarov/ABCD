...
use dds_domain::{
    body_types, DeviceJoinDocument, DomainDocument, SessionDocument, UserAuthAttestation,
};
...
impl<S: TokenStore + dds_store::traits::RevocationStore> LocalService<S> {
    ...
    /// Get the node identity URN.

    /// Enroll a user with FIDO2 attestation.
    pub fn enroll_user(&mut self, req: EnrollUserRequest) -> Result<EnrollmentResult, ServiceError> {
        ...
        doc.embed(&mut payload).map_err(|e| ServiceError::Domain(e.to_string()))?;

        let token = Token::sign(payload, &user_ident.signing_key)
            .map_err(|e| ServiceError::Token(e.to_string()))?;
        let cbor = token.to_cbor().map_err(|e| ServiceError::Token(e.to_string()))?;
        self.store.put_token(&token).map_err(|e| ServiceError::Store(e.to_string()))?;
    ...
    /// Enroll a device.
    pub fn enroll_device(&mut self, req: EnrollDeviceRequest) -> Result<EnrollmentResult, ServiceError> {
        let device_ident = Identity::generate(&req.label, &mut rand::rngs::OsRng);
        let doc = DeviceJoinDocument {
            device_id: req.device_id, hostname: req.hostname,
            os: req.os, os_version: req.os_version,
            tpm_ek_hash: req.tpm_ek_hash, org_unit: req.org_unit, tags: req.tags,
        };
    ...
    pub fn issue_session(&mut self, req: SessionRequest) -> Result<SessionResult, ServiceError> {
        ...
        let granted = self.trust_graph.purposes_for(&req.subject_urn, &self.trusted_roots);
        ...
        let mut payload = TokenPayload {
            ...
            iat: now_epoch(), exp: Some(expires_at),
        ...
        Ok(SessionResult { session_id, token_cbor: cbor, expires_at })
    }
    ...
    pub fn evaluate_policy(&self, subject_urn: &str, resource: &str, action: &str) -> PolicyResult {
        let decision = self.policy_engine.evaluate(
            subject_urn, resource, action, &self.trust_graph, &self.trusted_roots,
        );
        PolicyResult {
