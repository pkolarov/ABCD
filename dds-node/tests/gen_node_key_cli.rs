...
fn gen_node_key_creates_key_files() {
    ...
    assert!(
        data.join("p2p_key.bin").exists(),
        "p2p_key.bin should be created"
    );
    assert!(
        data.join("epoch_keys.cbor").exists(),
        "epoch_keys.cbor should be created"
    );
}
...
fn gen_node_key_stdout_reports_peer_id_and_kem_pubkey() {
    ...
    let (ok, stdout, _stderr) =
        run_capture(dds_node_bin().args(["gen-node-key", "--data-dir", data.to_str().unwrap()]));
    assert!(ok, "gen-node-key should succeed");
    assert!(
        stdout.contains("peer_id:"),
        "stdout should include peer_id; got: {stdout}"
    );
    assert!(
        stdout.contains("kem_pubkey_hex:"),
        "stdout should include kem_pubkey_hex; got: {stdout}"
    );
    assert!(
        stdout.contains("data_dir:"),
        "stdout should include data_dir; got: {stdout}"
    );
}
...
fn gen_node_key_kem_pubkey_is_hex() {
    ...
    assert!(
        !hex_val.is_empty(),
        "kem_pubkey_hex must not be empty; line: {kem_line}"
    );
    assert!(
...
fn gen_node_key_is_idempotent() {
    ...
    let peer_id_1 = stdout1
        .lines()
        .find(|l| l.contains("peer_id:"))
        .unwrap_or("");
    let peer_id_2 = stdout2
        .lines()
        .find(|l| l.contains("peer_id:"))
        .unwrap_or("");
    assert_eq!(
        peer_id_1, peer_id_2,
        "peer_id must be stable across repeated gen-node-key calls"
    );
}
...
fn gen_node_key_creates_data_dir_if_absent() {
    ...
    assert!(
        status.success(),
        "gen-node-key should create a missing data dir"
    );
