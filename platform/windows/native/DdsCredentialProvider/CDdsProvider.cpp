...
// ICredentialProviderSetUserArray — required on Windows 8+/10/11 for LogonUI to
// accept the provider.  We receive the system user array but manage our own tiles,
// so we just acknowledge the call.
HRESULT CDdsProvider::SetUserArray(ICredentialProviderUserArray* users)
{
    CPLog("SetUserArray: users=%p", users);
    return S_OK;
}
...
HRESULT CDdsProvider_CreateInstance(REFIID riid, void** ppv)
{
    ...
    CPLog("CDdsProvider_CreateInstance: enter");
    HRESULT hr;
    ...
    if (pProvider)
    {
        ...
        CPLog("CDdsProvider_CreateInstance: QueryInterface hr=0x%08X", (unsigned)hr);
        pProvider->Release();
    }
    else
    {
    ...
    CPLog("CDdsProvider_CreateInstance: returning hr=0x%08X", (unsigned)hr);
    return hr;
}
