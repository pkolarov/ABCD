...
# S-1-5-21-...-1001) — that primary SID needs to be in the allowlist
# for the upcoming /v1/enroll/device + /v1/admin/* calls (steps 7-8).
# DdsAuthBridge / DdsPolicyAgent run as LocalSystem so they don't
# need the entry; this is purely the bootstrap operator's seat.
$operatorSid = ([Security.Principal.WindowsIdentity]::GetCurrent()).User.Value
Write-Host "  Operator SID: $operatorSid (added to windows_admin_sids)"
$tomlContent = @"
...
# Operator SID — needed for bootstrap-time /v1/enroll/device + /v1/admin/*.
# LocalSystem (S-1-5-18) is implicit; everyone else must be listed here.
windows_admin_sids = ["$operatorSid"]
