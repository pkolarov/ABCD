...
# `dds` (the CLI). Every action that grants or revokes trust requires
...
holds_admin_key_for() {
  local urn="$1" hash
  hash="$(printf '%s' "${urn}" | shasum -a 256 | awk '{print $1}')"
...
open(p, 'w').write(s)
PY
    launchctl kickstart -k system/com.dds.node
  ...
  printf "Label for this admin [$(hostname -s)-admin]: "
  read -r LABEL
  ...
  local setup_log="/tmp/dds-admin-setup-${ts}.log"
  set +e
