...
echo ""
if [[ -n "${MULTIADDR:-}" ]]; then
echo "  On the new node's config, set:"
echo "    bootstrap_peers = [\"${MULTIADDR}\"]"
echo ""
fi
...
echo "    3. Edit C:\\ProgramData\\DDS\\dds.toml with domain info"
if [[ -n "${MULTIADDR:-}" ]]; then
echo "       Set: bootstrap_peers = [\"${MULTIADDR}\"]"
fi
echo "    4. Start DdsNode service"
echo "    5. Wait ~60s for gossip sync"
echo "    6. Check: GET http://localhost:5551/v1/enrolled-users"
echo ""
