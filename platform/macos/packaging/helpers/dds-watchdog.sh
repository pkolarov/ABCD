...
#
# macOS port of platform/windows/installer/scripts/Watchdog-DdsNode.ps1
# (watchdog plan Step 7). Run every 5 minutes by
# /Library/LaunchDaemons/com.dds.watchdog.plist (StartInterval 300).
#
...
#
# State: ${DDS_ROOT}/watchdog-state.json
# Logs:  syslog via logger -t dds-watchdog (Warning on restart, Error on
#        restart-failure or cap-hit — mirrors EventIds 1000/1001/1002).
set -uo pipefail

DDS_ROOT="/Library/Application Support/DDS"
API_SOCK="${DDS_ROOT}/dds.sock"
STATE_FILE="${DDS_ROOT}/watchdog-state.json"
REQUIRED_CONSECUTIVE="${DDS_WATCHDOG_REQUIRED_CONSECUTIVE:-3}"
MAX_RESTARTS_24H="${DDS_WATCHDOG_MAX_RESTARTS_24H:-3}"
RESTART_CMD=(launchctl kickstart -k system/com.dds.node)

# Poll the node. Any failure to read well-formed status with both
# admission fields leaves CONNECTED/ADMITTED empty => UNKNOWN.
CONNECTED=""
ADMITTED=""
...
# below only dispatches the action.
...
case "${ACTION}" in
  ...
  capped)
    ...
    ;;
  *)
    # 'none' (or empty on an internal error) — nothing beyond the state
    # already persisted.
    ;;
esac
exit 0
